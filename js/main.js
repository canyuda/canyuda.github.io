(function (w, d) {
    ...
    var body = d.body,
        ...
        header = $('#header'),
        mask = $('#mask'),
        asideToc = $("#post-toc-id");
        menuToggle = $('#menu-toggle'),
        tocToggle = $('#toc-toggle'),    // todo 找个时间, tocToggle替换成post里面得到东西
        menuOff = $('#menu-off'),
        loading = $('#loading'),
    ...
    var Blog = {
        ...
        toggleToc: function(){
            if (asideToc){
                var flag = asideToc.classList.contains('post-widget');
                if (flag) {
                    asideToc.classList.remove('post-widget')
                    article.classList.remove('post-article')
                    asideToc.classList.add('post-widget-hide')
                    article.classList.add('post-article-hide')
                    
                } else {
                    article.classList.remove('post-article-hide')
                    asideToc.classList.remove('post-widget-hide')
                    article.classList.add('post-article')
                    asideToc.classList.add('post-widget')
                }
            } else {
                console.log("not found asideToc")
            }
        },
    ...
    // todo 找个时间, tocToggle替换成post里面得到东西
    if (tocToggle){
        tocToggle.addEventListener(even, function (e) {
            Blog.toggleToc(true);
            e.preventDefault();
        }, false);
    }
